...
class PersistentLabelStore:
    ...
    def remove_by_label(self, label: str) -> int:
        """Remove all cached embeddings for a class name. Returns count removed.

        Used when the user deletes an unused/unwanted discovered class so the
        auto-label cache cannot resurrect that name on later tiles.
        """
        if not label:
            return 0
        ...
        keep_r: List[bool] = []
        removed = 0
        for emb, lab, rel in zip(self._embeddings, self._labels, self._relevances):
            if lab == label:
                removed += 1
                continue
            keep_e.append(emb)
        ...
        if removed:
            ...
            self._labels = keep_l
            self._relevances = keep_r
            self._rebuild_index()
            try:
                self.save()
            except Exception as e:
                print(f"[LabelStore] remove_by_label save failed: {e}")
        return removed
